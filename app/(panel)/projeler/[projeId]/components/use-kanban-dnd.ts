// Sprint 3 / S3-6 — Kanban pano DnD orkestrasyon hook'u.
// ADR-0032 plan'ında "UI/logic ayır": pure logic kanban-dnd.ts'te (test edilebilir
// saf fonksiyonlar), bu hook UI komponenti'nden DnD state + handler'larını çıkarır.
// Pano dosyası UI'a (render JSX) ve form state'e (yeni liste / modal aç) odaklanır.
//
// Hook'un sorumluluğu:
//   - dnd-kit sensor + collision detection wiring
//   - Drag state (aktifDrag, kartPlaceholder)
//   - Stable handler'lar (useEvent ile DndContext re-init önlenir)
//   - Mutation orkestrasyonu: kart taşı, liste sırala (cache restore + arşiv toast)
...
// =====================================================================
// Modül seviyesi sabitler — referansları render'lar arasında stable kalsın
// (Kural T.6 / dnd-kit context re-init önleme).
// =====================================================================
...
type UseKanbanDndGirdi = {
  projeId: string;
  // TanStack Query queryKey — useMemo'lanmış stable referans (kural T.5).
  anahtar: DetayKey;
  detay: ProjeDetayOzeti;
  // Daraltılmış liste id'leri — collision detection daraltılmışları drop
  // hedefinden filtreler (defansif; useDroppable disabled flag'i ilk savunma).
  daraltilmisListeler: ReadonlySet<string>;
};

export type UseKanbanDndCikti = {
  sensorlar: ReturnType<typeof useSensors>;
  collisionDetection: CollisionDetection;
  // DragOverlay için — sürüklenen kart/liste'nin "hayalet" gösterimi.
  aktifDrag: DragVeri;
  // KanbanListe placeholder feedback için — dragOver sırasında transient.
  kartPlaceholder: KartPlaceholder | null;
  // DndContext'e geçilen stable handler'lar (useEvent ile sarılı).
  dragBaslat: (e: DragStartEvent) => void;
  dragUzerinde: (e: DragOverEvent) => void;
  dragBitti: (e: DragEndEvent) => void;
  dragIptal: (e: DragCancelEvent) => void;
...
export function useKanbanDnd({
  ...
  detay,
  daraltilmisListeler,
}: UseKanbanDndGirdi): UseKanbanDndCikti {
  const istemci = useQueryClient();

  // Trello tarzı: 5px hareket sonrası mouse drag aktive (yanlışlıkla
  // sürüklemeyi engeller). Touch: 200ms basılı tutma + 8px tolerans.
  ...
  // Sürüklenen kartın "şu an" hangi listede / hangi pozisyonda olduğunu izlemek
  // için. onDragOver sırasında bu güncellenir, görsel feedback verir.
  ...
  // Kart sürüklemede: önce pointerWithin (hassas, kart üzerinde imleç var mı),
  // sonra rectIntersection. Liste sürüklemede: closestCenter benzeri (yatay).
  const collisionDetection: CollisionDetection = React.useCallback(
    (args) => {
      ...
      if (aktifTip === "liste") {
        // Liste sürüklenirken sadece diğer listeleri hedefle (liste body veya
        // kart hedef değil).
        const sadeceListeler = args.droppableContainers.filter(
          (d) => d.data.current?.tip === "liste",
        );
        const ilk = pointerWithin({
          ...args,
          droppableContainers: sadeceListeler,
        });
      ...
      if (aktifTip === "kart") {
        // Kart sürüklenirken: önce hangi LİSTE üzerindeyiz onu bul (pointer
        // ile), sonra o liste içindeki kartlardan en yakın merkeze sahip olanı
        // seç. Bu pattern dnd-kit multi-container örneğinin sadeleştirilmiş
        // hali — pointer 1px titremesi `closestCenter` ile absorb edilir.
  ...
  const dragBaslat = React.useCallback(
    (e: DragStartEvent) => {
  ...
  const dragUzerinde = React.useCallback(
    (e: DragOverEvent) => {
      ...
      kartPlaceholderGuncelle({
        ...
        yukseklik:
          Math.round(
            e.active.rect.current.initial?.height ?? e.over.rect.height,
          ) || null,
      });
    },
    [kartPlaceholderGuncelle],
  ...
  const dragBitti = React.useCallback(
    (e: DragEndEvent) => {
      const aktif = aktifDragRef.current;
      const detay = detayRef.current;
      setAktifDrag(null);
      ...
      if (aktif.tip === "liste") {
        ...
        const eskiIdx = detay.listeler.findIndex(
          (l) => l.id === aktif.liste.id,
        );
        ...
        if (arsivIdx !== -1 && arsivIdx !== yenidenSiralanmis.length - 1) {
  ...
  const dragIptal = React.useCallback(
    (_e: DragCancelEvent) => {
      setAktifDrag(null);
      kartPlaceholderGuncelle(null);
      baslangicaDon();
  ...
  // useEvent — dnd-kit'in resmi stable handler primitive'i (Kural T.5).
  ...
  return {
    ...
    aktifDrag,
    kartPlaceholder,
    dragBaslat: dragBaslatStable,
    dragUzerinde: dragUzerindeStable,
    dragBitti: dragBittiStable,
    dragIptal: dragIptalStable,
    modifiers: MODIFIERS,
  };
}
