...
## İkinci Düzeltme — "Branch Not Match" (2026-05-11, commit 304c121)

T1 ve T2 ilk smoke test'lerinde Actions yeşil görünüyordu ama Dokploy panelinde yeni deployment **başlamıyordu**. Log incelemesi:

```
Status: 301
Body: {"message":"Branch Not Match"}
```

Sebep: Dokploy webhook handler `extractBranchName(req.headers, req.body)` ile branch çıkarıyor (kaynak: `apps/dokploy/pages/api/deploy/[refreshToken].ts` @ canary). Boş POST → branch null → `application.branch` ile eşleşmez → 301. `curl -fsS` 3xx'i fail saymadığı için Actions yanıltıcı yeşildi.

Düzeltme: Webhook çağrısına GitHub push event payload eklendi:
```yaml
curl -sS -X POST \
  ...
  "$WEBHOOK_URL"
# + status code != 200 ise explicit fail
```

## Test Sonuçları (2026-05-11 end-to-end)

| Senaryo | Değişiklik | deploy-app | deploy-socket | Dokploy yanıtı |
|---------|-----------|:---:|:---:|---|
| T10 | `README.md` | skipped | skipped | — |
| T1.v2 | `app/page.tsx` | ✅ 200 | skipped | App: "Application deployed successfully" |
| T2.v2 | `socket-server/index.ts` | skipped | ✅ 200 | Socket: "Application deployed successfully" |
| workflow_dispatch `target=both` | manuel | ✅ 200 | ✅ 200 | İkisi de "deployed successfully" |

End-to-end pipeline ayrımı doğrulandı. Plan §10.1 K1.1 (≥9/10 senaryo) karşılandı.
