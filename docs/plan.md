...
| 🔄 **S6 — Görünüm Polish + Global Search + Filtre + Timeline** | 6 gün | Pano/Liste görünümleri arasında geçiş ✅, proje içi filtre ⏳. **Global search: tsvector kolonları + GIN index + komut paleti (Cmd/Ctrl+K + Cmd/Ctrl+Space) UI (1.5/A) ✅**, fuzzy match (pg_trgm) ✅, Türkçe stemmer ✅, Altay tasarım paritesi (tab bar + renkli ikon + highlight + skeleton) ✅. **Aktivite timeline (#10) ✅**. (Takvim + audit log arama v1.1'e ertelendi) |
